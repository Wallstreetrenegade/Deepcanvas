...
_PUBLIC_WEB_METHODS = frozenset({"auth.signup", "auth.signin", "auth.me", "auth.signout"})


def _web_auth_required() -> bool:
    raw = os.getenv("DEEPCANVAS_REQUIRE_WEB_AUTH") or os.getenv("JIUWENCLAW_REQUIRE_WEB_AUTH")
...
class WebChannel(BaseChannel):
    ...
    async def _process_request(self, *args: Any) -> Any:
        ...
        logger.info(
            "WebChannel handshake check path=%s origin=%s allowed=%s",
            path,
            origin,
            allowed,
        )
        if allowed:
            return None
        logger.warning(
            "WebChannel handshake rejected path=%s origin=%s reason=origin_not_allowed",
            path,
            origin,
        )
    ...
    async def _connection_handler(self, ws: Any, path: str | None = None) -> None:
        ...
        query_token = (query.get("auth_token") or query.get("token") or [""])[0]
        query_user = auth.authenticate_token(query_token) if query_token else None
        setattr(ws, "_exclaw_auth_token", query_token if query_user else "")
    ...
    async def _handle_raw_message(self, ws: Any, raw: str, query: dict[str, list[str]]) -> None:
        ...
        request_token = str(
            ...
            or params.get("token")
            or getattr(ws, "_exclaw_auth_token", "")
            or ""
        ).strip()
        user = getattr(ws, "_exclaw_auth_user", None)
        if request_token and (not user or request_token != getattr(ws, "_exclaw_auth_token", "")):
            user = auth.authenticate_token(request_token)
            setattr(ws, "_exclaw_auth_token", request_token if user else "")
            setattr(ws, "_exclaw_auth_user", user)

        if _web_auth_required() and method not in _PUBLIC_WEB_METHODS and not user:
            await self.send_response(
                ws,
                req_id,
                ok=False,
        ...
        if user and session_id and session_id != req_id and method not in {"session.create", "session.list"}:
            try:
                from jiuwenclaw.agentserver.session_metadata import get_session_metadata

                session_meta = get_session_metadata(session_id)
                if session_meta and session_meta.get("user_id") and session_meta.get("user_id") != user["id"]:
                    await self.send_response(ws, req_id, ok=False, error="session not found", code="NOT_FOUND")
                    return
            except Exception:
                logger.warning("WebChannel session ownership check failed", exc_info=True)
        ...
        auth_context_token = auth.set_current_user(user)
        if self._on_message_cb is not None:
            try:
                ...
                auth.reset_current_user(auth_context_token)
        else:
            try:
        ...
        if handler is not None:
            ...
            try:
                ...
                await self.send_response(ws, req_id, ok=False, error=str(e), code=e.code)
            except Exception as e:

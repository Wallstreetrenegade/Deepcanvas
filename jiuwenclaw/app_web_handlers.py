...
_CONFIG_SET_ENV_MAP = {
    ...
    "features_model": "FEATURES_MODEL_NAME",
    "features_api_base": "FEATURES_API_BASE",
    "features_api_key": "FEATURES_API_KEY",
    # video 模型
...
def _available_model_providers() -> list[str]:
    """Return the union of ProviderType enum values and runtime-registered
    client provider keys. Runtime registrations (see jiuwen_core_patch) are the
    source of truth for which providers can actually service a request, so we
    include them here so validators don't block user-configurable providers
    such as Gemini, Anthropic, Google, FalAI or ElevenLabs.
    """
    providers: list[str] = [provider.value for provider in ProviderType]
    try:
        from openjiuwen.core.foundation.llm.model import _CLIENT_TYPE_REGISTRY
        for name in _CLIENT_TYPE_REGISTRY.keys():
            if name not in providers:
                providers.append(name)
    except Exception:  # noqa: BLE001 - validator must never crash on import issues
        pass
    return providers


def _canonical_model_provider(provider: Any) -> str:
    text = str(provider or "").strip()
    if not text:
        return ""
    for known in _available_model_providers():
        if text.lower() == str(known).lower():
            return str(known)
    return text


def _provider_alias_env_updates(provider: str, api_key: str, api_base: str) -> dict[str, str]:
    provider_key = provider.strip().lower()
    updates: dict[str, str] = {}
    if provider_key == "openrouter":
        updates["OPENROUTER_API_KEY"] = api_key
        updates["OPENROUTER_API_BASE"] = api_base or "https://openrouter.ai/api/v1"
...
def _register_web_handlers(bind: WebHandlersBindParams) -> None:
    ...
    async def _config_get(ws, req_id, params, session_id):
        ...
        # Feature agents inherit the main chat model unless their dedicated
        # fields are explicitly configured. Showing the inherited values in
        # Settings prevents App Builder from looking disconnected from the
        # default model controls.
        for feature_key, default_key in (
            ("features_provider", "model_provider"),
            ("features_model", "model"),
            ("features_api_base", "api_base"),
            ("features_api_key", "api_key"),
        ):
            if not payload.get(feature_key):
                payload[feature_key] = payload.get(default_key, "")
    ...
    async def _config_set(ws, req_id, params, session_id):
        ...
        for param_key, env_key in _CONFIG_SET_ENV_MAP.items():
            if param_key not in params:
                continue
            raw_val = raw_params.get(param_key)
            raw_text = "" if raw_val is None else str(raw_val).strip()
            current_text = os.getenv(env_key) or ""
            try:
                from jiuwenclaw.extensions.registry import ExtensionRegistry
                if (("api_key" in param_key.lower() or "token" in param_key.lower())
                        and current_text
                        and ExtensionRegistry.get_instance().get_crypto_provider()):
                    current_text = ExtensionRegistry.get_instance().get_crypto_provider().decrypt(current_text)
            except Exception:  # noqa: BLE001
                pass
            ...
            if param_key.endswith("_provider") and val:
                val = _canonical_model_provider(val)
        ...
        default_model_changed = any(
            key in env_updates
            for key in ("MODEL_PROVIDER", "MODEL_NAME", "API_BASE", "API_KEY")
        )
        feature_params_changed = any(key in raw_params for key in (
            "features_provider", "features_model", "features_api_base", "features_api_key",
        ))
        if default_model_changed and not feature_params_changed:
            provider = str(env_updates.get("MODEL_PROVIDER") or os.getenv("MODEL_PROVIDER") or "").strip()
            model = str(env_updates.get("MODEL_NAME") or os.getenv("MODEL_NAME") or "").strip()
            api_base = str(env_updates.get("API_BASE") or os.getenv("API_BASE") or "").strip()
            api_key = str(env_updates.get("API_KEY") or os.getenv("API_KEY") or "").strip()
            if provider:
                env_updates["FEATURES_PROVIDER"] = provider
            if model:
                env_updates["FEATURES_MODEL_NAME"] = model
            if api_base:
                env_updates["FEATURES_API_BASE"] = api_base
            if api_key:
    ...
    async def _config_validate_model(ws, req_id, params, session_id, max_tokens_bounds=None):
        ...
        model_provider = _canonical_model_provider(params.get("model_provider"))
        ...
        async def test_invoke(max_tokens: int):
            # Newer OpenAI models (gpt-5.x, o1, o3, etc.) use max_completion_tokens instead of max_tokens
            invoke_kwargs = {}
            normalized_model = model.split("/", 1)[-1]
            if model_provider.lower() == "openai" and normalized_model.startswith(("gpt-5", "o1", "o3", "o4")):
    ...
    async def _session_list(ws, req_id, params, session_id):
        ...
        sessions, total = get_all_sessions_metadata(limit=limit, offset=offset, user_id=user["id"])
    ...
    async def _session_delete(ws, req_id, params, session_id):
        ...
        user = auth.require_current_user()
        meta = get_session_metadata(session_id_to_delete)
    ...
    async def _auth_signup(ws, req_id, params, session_id):
        ...
        try:
            payload = auth.signup(
                ...
                str(params.get("displayName") or params.get("display_name") or ""),
            )
            await channel.send_response(ws, req_id, ok=True, payload=payload)
        except auth.AuthError as exc:
            await channel.send_response(ws, req_id, ok=False, error=str(exc), code=exc.code)

    async def _auth_signin(ws, req_id, params, session_id):
        if not isinstance(params, dict):
            await channel.send_response(ws, req_id, ok=False, error="params must be object", code="BAD_REQUEST")
            return
        try:
            payload = auth.login(str(params.get("email") or ""), str(params.get("password") or ""))
            await channel.send_response(ws, req_id, ok=True, payload=payload)
        except auth.AuthError as exc:
            await channel.send_response(ws, req_id, ok=False, error=str(exc), code=exc.code)

    async def _auth_me(ws, req_id, params, session_id):
        user = auth.get_current_user()
        if not user and isinstance(params, dict):
            user = auth.authenticate_token(str(params.get("auth_token") or params.get("token") or ""))
        await channel.send_response(ws, req_id, ok=True, payload={"user": user})

    async def _auth_signout(ws, req_id, params, session_id):
        token = ""
        if isinstance(params, dict):
            token = str(params.get("auth_token") or params.get("token") or "")
        auth.logout(token)
        await channel.send_response(ws, req_id, ok=True, payload={"ok": True})

    async def _user_settings_get(ws, req_id, params, session_id):
        try:
            await channel.send_response(ws, req_id, ok=True, payload={"settings": auth.get_settings()})
        except auth.AuthError as exc:
            await channel.send_response(ws, req_id, ok=False, error=str(exc), code=exc.code)

    async def _user_settings_set(ws, req_id, params, session_id):
        if not isinstance(params, dict):
            await channel.send_response(ws, req_id, ok=False, error="params must be object", code="BAD_REQUEST")
            return
        raw_settings = params.get("settings", params)
        if not isinstance(raw_settings, dict):
            await channel.send_response(ws, req_id, ok=False, error="settings must be object", code="BAD_REQUEST")
            return
        try:
            await channel.send_response(
                ws,
                req_id,
                ok=True,
                payload={"settings": auth.update_settings(raw_settings)},
            )
        except auth.AuthError as exc:
            await channel.send_response(ws, req_id, ok=False, error=str(exc), code=exc.code)
    ...
    channel.register_method("auth.signup", _auth_signup)
    channel.register_method("auth.signin", _auth_signin)
    channel.register_method("auth.me", _auth_me)
    channel.register_method("auth.signout", _auth_signout)
    channel.register_method("user.settings.get", _user_settings_get)
    channel.register_method("user.settings.set", _user_settings_set)
    ...
    # PI Agent (features supervisor) - registers pi.* methods
    try:
        from jiuwenclaw.pi_agent import register_pi_handlers
    ...
    try:
        ...
        register_social_station_handlers(channel)
    except Exception as _ss_exc:  # noqa: BLE001
        logger.exception("[social.station] failed to register handlers: %s", _ss_exc)
    # App Builder - registers app.builder.* methods (AI-driven landing page / site builder)
    try:
        from jiuwenclaw.pi_agent.app_builder import register_app_builder_handlers
        register_app_builder_handlers(channel, agent_client=agent_client)
    except Exception as _ab_exc:  # noqa: BLE001
        logger.exception("[app.builder] failed to register handlers: %s", _ab_exc)
    # Project Flow - URL ingestion, connected AI prompts, and fal.ai generators
    try:
        from jiuwenclaw.pi_agent.project_flow import register_project_flow_handlers
        register_project_flow_handlers(channel)
    except Exception as _pf_exc:  # noqa: BLE001
        logger.exception("[project_flow] failed to register handlers: %s", _pf_exc)
    # Social Station Larry autonomous agent - social.larry.* methods (Auto tab)
    try:
        from jiuwenclaw.pi_agent.social_larry import register_social_larry_handlers
        register_social_larry_handlers(channel)
    except Exception as _sl_exc:  # noqa: BLE001
        logger.exception("[social.larry] failed to register handlers: %s", _sl_exc)
    # Storage - disk-backed user media/document library + drive connections
    try:
        from jiuwenclaw.pi_agent.storage import register_storage_handlers
        register_storage_handlers(channel)
    except Exception as _storage_exc:  # noqa: BLE001
        logger.exception("[storage] failed to register handlers: %s", _storage_exc)
    # Email - outbound mail transport + persisted templates/campaign state
    try:
        from jiuwenclaw.pi_agent.email import register_email_handlers

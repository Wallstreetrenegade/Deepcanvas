...
def _default_dist_dir() -> Path:
    ...
    # In source checkout mode, prefer the repo's frontend build. The user
    # workspace can contain an older copied dist from a previous run, and serving
    # that first masks active frontend fixes during local development.
    if not is_package_installation() and (package_dir / "web" / "dist").exists():
        return package_dir / "web" / "dist"

    # In package mode, prefer the copied runtime dist when available.
    if (root / "web" / "dist").exists():
        return root / "web" / "dist"

    # Fallback to package internal web/dist.
...
class _SpaStaticHandler(SimpleHTTPRequestHandler):
    ...
    def _is_mail_route(self) -> bool:
        return urlparse(self.path).path.startswith("/mail")
    ...
    def _proxy_http(self, target: str | None = None) -> None:
        parsed = urlparse(target or self.api_target)
    ...
    def _dispatch_proxy(self) -> bool:
        if self._is_mail_route() and self.mail_target:
            self._proxy_http(self.mail_target)
            return True
    ...
    def do_GET(self) -> None:  # noqa: N802
        ...
        if parsed.path == "/health":
            self._write_json(200, {"ok": True, "service": "deepcanvas-web"})
            return
...
def main() -> None:
    ...
    parser.add_argument(
        "--mail-target",
        default=os.getenv("PLUNK_WEB_PROXY_TARGET", "http://127.0.0.1:3000"),
        help="Override Plunk target for /mail (http/https).",
    )
    parser.add_argument(
        "--log-level",
        default="INFO",
    ...
    try:
        ...
        mail_target = _normalize_api_target(args.mail_target.strip())
    ...
    _ConfiguredHandler.mail_target = mail_target
    ...
    logger.info("[jiuwenclaw-web] /mail -> %s", mail_target)

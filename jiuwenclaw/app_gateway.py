...
class GatewayServer:
    ...
    async def _connection_handler(self, ws: Any, path: str | None = None) -> None:
        ...
        auth_token = (query.get("auth_token") or query.get("token") or [""])[0]
        try:
            setattr(ws, "_exclaw_auth_token", auth_token)
            setattr(ws, "_exclaw_auth_user", auth.authenticate_token(auth_token))
        except Exception:
            setattr(ws, "_exclaw_auth_token", "")
            setattr(ws, "_exclaw_auth_user", None)
    ...
    async def _handle_raw_message(self, ws: Any, raw: str, request_path: str, route: RouteConfig) -> None:
        ...
        if user and session_id and session_id != req_id and method not in {"session.create", "session.list"}:
            try:
                ...
                logger.warning("GatewayServer session ownership check failed", exc_info=True)

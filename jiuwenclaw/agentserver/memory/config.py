...
def _resolve_mode_memory(mode: str, config: Optional[Dict[str, Any]]) -> Dict[str, Any]:
    """Locate the memory block for the requested runtime mode."""
...
def get_memory_engine(config: Optional[Dict[str, Any]] = None) -> str:
    """Return memory engine policy: builtin | external | both | none."""
    cfg = config if config is not None else _load_config()
    memory_cfg = (cfg or {}).get("memory", {}) if isinstance(cfg, dict) else {}
    value = str(memory_cfg.get("engine") or "builtin").strip().lower()

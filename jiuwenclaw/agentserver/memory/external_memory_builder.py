...
"""Builder for ExternalMemoryRail."""
...
def build_external_memory_rail(
    ...
    try:
        try:
            from openjiuwen.harness.rails.memory import ExternalMemoryRail
        except Exception:
    ...
    try:
        if provider_name == "openjiuwen":
            ...
            provider = _load_plugin_provider(
                provider_name,
                ext_cfg.get("allowed_plugins") or None,
            )
    except Exception as exc:
        logger.warning(
...
def _build_openviking_provider(ext_cfg: Dict[str, Any]):
    ...
    openviking_cfg = ext_cfg.get("openviking") or {}
    endpoint = openviking_cfg.get("endpoint") or os.environ.get("OPENVIKING_ENDPOINT", "")
    api_key = openviking_cfg.get("api_key") or os.environ.get("OPENVIKING_API_KEY", "")
    account = openviking_cfg.get("account") or os.environ.get("OPENVIKING_ACCOUNT", "root")
    user = openviking_cfg.get("user") or os.environ.get("OPENVIKING_USER", "default")
...
def _load_plugin_provider(name: str, allowed: Optional[list] = None):
    try:
        ...
        logger.warning(
            "[ExternalMemoryBuilder] plugin '%s' requested but plugin_discovery is unavailable",
            name,
        )
        return None

...
class SkillManager:
    ...
    async def handle_skills_import_local(self, params: dict) -> dict:
        ...
        force = params.get("force", False)
        if not raw_path:
            return {"success": False, "detail": "missing parameter: path"}

        try:
            if self._is_http_download_target(str(raw_path)):
                return await self._import_skill_from_remote_archive(params)
            return self._import_skill_from_path(
                Path(raw_path),
                force=force,
                origin=str(raw_path),
                source="local",
            ...
            logger.exception("import_local failed: %s", exc)
    ...
    @staticmethod
    def _get_import_local_allowed_download_hosts() -> tuple[str, ...]:
        raw = os.environ.get("IMPORT_LOCAL_ALLOWED_DOWNLOAD_HOSTS", "")
        if raw.strip():
            hosts = tuple(item.strip().lower() for item in raw.split(",") if item.strip())
            if hosts:
                return hosts
        return _IMPORT_LOCAL_DEFAULT_ALLOWED_DOWNLOAD_HOSTS

    @staticmethod
    def _host_matches_rule(host: str, rule: str) -> bool:
        host_value = host.strip().lower()
        rule_value = rule.strip().lower()
        if not host_value or not rule_value:
            return False
        return fnmatch.fnmatch(host_value, rule_value)

    @classmethod
    def _assert_import_local_download_url_allowed(cls, url: str) -> None:
        parsed = urlparse(str(url or "").strip())
        if parsed.scheme != "https" or not parsed.hostname:
            raise ValueError("remote skill import requires an https URL")
        allowed_hosts = cls._get_import_local_allowed_download_hosts()
        if not any(cls._host_matches_rule(parsed.hostname, rule) for rule in allowed_hosts):
            raise ValueError(
                "remote skill import host is not allowed; set IMPORT_LOCAL_ALLOWED_DOWNLOAD_HOSTS to allow it"
            )

    @staticmethod
    def _safe_skill_dir_name(value: Any, fallback: str) -> str:
        raw = str(value or "").strip() or str(fallback or "").strip() or "skill"
        if "\x00" in raw:
            raise ValueError("skill name contains invalid null byte")
        raw = raw.replace("\\", "/")
        ...
        if not name:
            name = re.sub(r"[^A-Za-z0-9_.-]+", "-", str(fallback or "skill")).strip(".-")
        return (name or "skill")[:120]

    def _resolve_skill_dest(self, folder_name: str) -> Path:
        root = self._skills_dir.resolve()
        dest = (root / folder_name).resolve()
        try:
            dest.relative_to(root)
        except ValueError as exc:
            raise ValueError("skill destination escaped skills directory") from exc
        return dest

    @staticmethod
    def _safe_archive_target(root: Path, archive_name: str) -> Path:
        normalized = str(archive_name or "").replace("\\", "/")
        pure = PurePosixPath(normalized)
        if pure.is_absolute() or any(part in {"", ".", ".."} for part in pure.parts):
            raise ValueError(f"unsafe archive member path: {archive_name!r}")
        ...
        try:
            ...
            raise ValueError(f"unsafe archive member path: {archive_name!r}") from exc
        return target

    @classmethod
    def _extract_archive_bytes_to_dir(cls, content: bytes, dest: Path, url_hint: str = "") -> None:
        suffixes = "".join(Path(urlparse(url_hint).path).suffixes).lower()
        is_zip = zipfile.is_zipfile(io.BytesIO(content)) or suffixes.endswith(".zip")
        is_tar = suffixes.endswith((".tar", ".tar.gz", ".tgz", ".tar.bz2", ".tbz2", ".tar.xz", ".txz"))

        if is_zip:
            with zipfile.ZipFile(io.BytesIO(content), "r") as zip_ref:
                for info in zip_ref.infolist():
                    target = cls._safe_archive_target(dest, info.filename)
                    if info.is_dir():
                        target.mkdir(parents=True, exist_ok=True)
                        continue
                    target.parent.mkdir(parents=True, exist_ok=True)
                    with zip_ref.open(info, "r") as source, target.open("wb") as output:
        ...
        if is_tar:
            with tarfile.open(fileobj=io.BytesIO(content), mode="r:*") as tar_ref:
                for member in tar_ref.getmembers():
                    target = cls._safe_archive_target(dest, member.name)
                    if member.isdir():
                        target.mkdir(parents=True, exist_ok=True)
                        continue
        ...
        raise ValueError("unsupported remote skill archive format")

    def _import_skill_from_path(self, src: Path, *, force: bool, origin: str, source: str) -> dict:
        if not src.exists():
            return {"success": False, "detail": f"è·¯å¾„ä¸å­˜åœ¨: {origin}"}

        if src.is_file():
            meta = self._parse_skill_md(src)
            if meta is None:
                return {"success": False, "detail": "æ— æ³•è§£æž skill æ–‡ä»¶"}
            skill_name = str(meta.get("name") or src.stem).strip() or src.stem
            folder_name = self._safe_skill_dir_name(skill_name, src.stem)
            dest = self._resolve_skill_dest(folder_name)
            if dest.exists():
                if not force:
                    return {"success": False, "detail": f"skill {skill_name} å·²å­˜åœ¨"}
                _safe_rmtree(dest)
            ...
            if md is None:
                return {"success": False, "detail": f"ç›®å½•ä¸­æœªæ‰¾åˆ° SKILL.md: {origin}"}
            meta = self._parse_skill_md(md) or {}
            skill_name = str(meta.get("name") or src.name).strip() or src.name
            folder_name = self._safe_skill_dir_name(skill_name, src.name)
            dest = self._resolve_skill_dest(folder_name)
            if dest.exists():
                if not force:
                    return {"success": False, "detail": f"skill {skill_name} å·²å­˜åœ¨"}
            ...
            return {"success": False, "detail": f"ä¸æ”¯æŒçš„è·¯å¾„ç±»åž‹: {origin}"}
        ...
        self._refresh_agent_data_indexes()
        return {"success": True, "skill": {"name": skill_name, "source": source}}

    async def _import_skill_from_remote_archive(self, params: dict) -> dict:
        url = str(params.get("path", "")).strip()
        force = bool(params.get("force", False))
        expected_sha256 = str(params.get("sha256", "")).strip().lower()
        source = str(params.get("source", "remote")).strip() or "remote"

        self._assert_import_local_download_url_allowed(url)

        try:
            import httpx

            async with httpx.AsyncClient(timeout=_IMPORT_LOCAL_REMOTE_TIMEOUT, follow_redirects=False) as client:
                response = await client.get(url)
                response.raise_for_status()
                content = response.content
        except Exception as exc:
            return {"success": False, "detail": f"remote skill download failed: {exc}"}

        if not content:
            return {"success": False, "detail": "remote skill archive was empty"}
        if expected_sha256:
            actual = hashlib.sha256(content).hexdigest()
            if actual != expected_sha256:
                return {"success": False, "detail": "remote skill archive sha256 mismatch"}

        with tempfile.TemporaryDirectory(prefix="jiuwenclaw_skill_import_") as tmpdir:
            tmp_path = Path(tmpdir)
            self._extract_archive_bytes_to_dir(content, tmp_path, url)
            skill_dir = self._locate_skill_dir(tmp_path)
            if skill_dir is None:
                return {"success": False, "detail": "remote archive did not contain SKILL.md"}
            return self._import_skill_from_path(skill_dir, force=force, origin=url, source=source)

...
def get_a2ui_config(config: dict[str, Any] | None = None) -> A2UIConfig:
    ...
    env_enabled = _first_env(
        "DEEPCANVAS_A2UI_ENABLED",
        "JIUWENCLAW_A2UI_ENABLED",
        "JIUWENSWARM_A2UI_ENABLED",
    )
    ...
    protocol_version = str(
        _first_env(
            "DEEPCANVAS_A2UI_PROTOCOL_VERSION",
            "JIUWENCLAW_A2UI_PROTOCOL_VERSION",
            "JIUWENSWARM_A2UI_PROTOCOL_VERSION",
        )
    ...
    env_smoke = _first_env(
        "DEEPCANVAS_A2UI_SMOKE_TOOLS",
        "JIUWENCLAW_A2UI_SMOKE_TOOLS",
        "JIUWENSWARM_A2UI_SMOKE_TOOLS",
    )
...
def get_current_a2ui_config() -> A2UIConfig:
    """Read A2UI config from the active Deepcanvas/JiuwenClaw runtime config."""

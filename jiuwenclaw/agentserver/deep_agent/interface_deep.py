...
try:
    ...
    from openjiuwen.harness.rails.memory import MemoryRail, CodingMemoryRail
...
except ImportError:  # Compatibility with older openjiuwen versions.
    try:
...
def _model_client_config_looks_usable(mcc: dict[str, Any]) -> bool:
...
class JiuWenClawDeepAdapter:
    ...
    @staticmethod
    async def _drain_pending_approval_events_compat(rail: Any) -> list[Any]:
        """Handle SkillEvolutionRail drain methods that are sync or async."""
        if rail is None or not hasattr(rail, "drain_pending_approval_events"):
            return []
    ...
    def _prioritize_paid_search_tool_card(self) -> None:
        """Keep paid_search before free_search when both cards are registered."""
        if not self._tool_cards:
            return
        paid_cards = [
    ...
    async def _get_tool_cards(self, agent_id: str):
        ...
        # Features / PI tools: give the main agent visibility into the
        # Kanban / CRM / Project Flow / Social Station workspaces.
        try:
            ...
            logger.info(
                "[JiuWenClawDeepAdapter] feature tools registered: tools=%s",
                _feature_tool_names,
            )
        except Exception as exc:
    ...
    def _has_valid_model_config(self) -> bool:
        """Check whether the active model config has real credentials."""
        if self._model is not None:
            mcc = getattr(self._model, "model_client_config", None)
            if isinstance(mcc, ModelClientConfig):
                if _model_client_config_looks_usable({
                    "api_key": mcc.api_key,
                    "api_base": getattr(mcc, "api_base", None),
                }):
                    return True

        try:
            mcc = get_config().get("models", {}).get("default", {}).get("model_client_config", {})
            if isinstance(mcc, dict) and _model_client_config_looks_usable(mcc):
                return True
        except Exception as exc:  # noqa: BLE001
            logger.warning("[JiuWenClawDeepAdapter] model config validity check failed: %s", exc)

        return False

    def _bind_runtime_auth_context(self, request: AgentRequest) -> Token | None:
        """Bind the web user id while DeepAgent tools run."""
        params = request.params if isinstance(request.params, dict) else {}
        user_id = str(params.get("user_id") or "").strip()
        if not user_id:
            return None
        return auth.set_current_user({
            "id": user_id,
            "email": str(params.get("user_email") or ""),
            "displayName": str(params.get("display_name") or params.get("user_name") or user_id),
        })

    def _reset_runtime_auth_context(self, token: Token | None) -> None:
    ...
    async def _handle_evolve_command(self, query: str, session_id: str) -> dict[str, Any]:
        ...
        events = await self._drain_pending_approval_events_compat(rail)
        if not events:
            return {
    ...
    async def process_message_impl(
        ...
        token_auth = self._bind_runtime_auth_context(request)
        ...
        try:
            ...
            self._reset_runtime_auth_context(token_auth)
    ...
    async def process_message_stream_impl(
        ...
        try:
            ...
            if self._skill_evolution_rail is not None:
                pending_approval_events = await self._drain_pending_approval_events_compat(
                    self._skill_evolution_rail
                )
                for evt in pending_approval_events:
    ...
    async def _handle_external_memory_rail_by_config(self):
        """Register or unregister ExternalMemoryRail based on config."""
        ...
        if is_external_memory_enabled(config):
            ...
            try:
                ...
                logger.error(
                    "[JiuWenClawDeepAdapter] ExternalMemoryRail register failed: %s",
                    exc,
                )
            ...
            try:
                ...
                logger.warning(
                    "[JiuWenClawDeepAdapter] ExternalMemoryRail unregister failed: %s",
                    exc,
                )
            self._external_memory_rail = None

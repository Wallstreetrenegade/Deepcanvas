...
function ThemeToggle() {
  ...
  const [theme, setTheme] = useState(() => {
    const themePreferenceVersion = localStorage.getItem('themePreferenceVersion');
    let savedTheme = localStorage.getItem('theme');
    if (themePreferenceVersion !== '2') {
      savedTheme = 'dark';
      localStorage.setItem('theme', 'dark');
      localStorage.setItem('themePreferenceVersion', '2');
    }
...
const AUTH_STORAGE_KEY = 'exclaw_auth_session';
...
function getStoredSessionId(storageKey = SESSION_STORAGE_KEY): string | null {
  try {
...
function storeSessionId(sessionId: string | null, storageKey = SESSION_STORAGE_KEY) {
  try {
...
function AuthScreen({
  ...
  const submit = useCallback(async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    setLoading(true);
    try {
      const payload = await webRequest<AuthSession>(
...
function AppContent({ authSession, onSignOut }: { authSession: AuthSession; onSignOut: () => void }) {
  const { t } = useTranslation();
  const userSessionStorageKey = `${SESSION_STORAGE_KEY}_${authSession.user.id}`;
  // 优先使用存储的会话 ID，避免每次刷新创建新会话
  const [sessionId, setSessionId] = useState<string>(() => {
    const stored = getStoredSessionId(userSessionStorageKey);
    return stored || 'new';
  });
  ...
  const {
    ...
    onConnect: (payload) => {
      const currentStored = getStoredSessionId(userSessionStorageKey);
      if (payload.session_id) {
        ...
        if (!currentStored) {
          ...
          storeSessionId(payload.session_id, userSessionStorageKey);
        } else {
        ...
        storeSessionId(fallbackSid, userSessionStorageKey);
      }
    },
    onDisconnect: () => {
  ...
  const saveConfigAndRestart = useCallback(async (updates: Record<string, string>) => {
    ...
    setServerConfig((current) => ({ ...(current ?? {}), ...updates }));
    setConfigError(null);
  ...
  // Keep the hosted product pinned to English.
  useEffect(() => {
    if (!isConnected) return;
    i18n.changeLanguage('en');
    void webRequest('locale.set_conf', { preferred_language: 'en' }).catch(() => {});
  }, [isConnected]);
  ...
  const handleNewSession = useCallback(async () => {
    ...
    try {
      ...
      storeSessionId(createdSid, userSessionStorageKey);
      // 保持当前模式
      if (switchMode) {
        try {
  ...
  const handleRestoreSession = useCallback(
    (targetSessionId: string) => {
      ...
      setCurrentSession(null);
      storeSessionId(targetSessionId, userSessionStorageKey);
      setActiveNav('chat');
    ...
    <div className="shell" data-testid="app-shell" data-session-id={sessionId}>
      ...
      <header className="topbar">
        ...
        <div className="topbar-actions">
          ...
          <div className="user-menu" title={authSession.user.email}>
            <span className="user-menu__name">{authSession.user.displayName || authSession.user.email}</span>
            <button type="button" className="user-menu__logout" onClick={onSignOut}>
              Sign out
            </button>
          </div>
        </div>
...
function App() {
  ...
  const handleSignOut = useCallback(() => {
    ...
    if (token) {
      void webRequest('auth.signout', { auth_token: token }).catch((error) => {
  ...
  return (
    <ErrorBoundary>
      {authSession ? (
        <AppContent authSession={authSession} onSignOut={handleSignOut} />
      ) : (

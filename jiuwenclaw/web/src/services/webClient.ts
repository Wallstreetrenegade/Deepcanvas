...
function isDevWsLoggingEnabled(): boolean {
  if (!import.meta.env.DEV) {
    return false;
  }
  try {
    return window.localStorage.getItem('deep_canvas_ws_debug') === '1';
  } catch {
    return false;
  }
}
...
class WebClient {
  ...
  async connect(options: WebConnectOptions = {}): Promise<void> {
    const previousAuthToken = this.lastConnectOptions.authToken || '';
    this.lastConnectOptions = options;
    if (this.ws?.readyState === WebSocket.OPEN) {
      if ((options.authToken || '') === previousAuthToken) {
        return;
      }
      await this.disconnect('auth changed');
    }
  ...
  async request<T = unknown>(
    ...
    if (this.lastConnectOptions.authToken) {
      message.auth_token = this.lastConnectOptions.authToken;
    }

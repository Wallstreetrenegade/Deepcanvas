...
export function normalizeModelSelection(bucket: ProviderBucket, provider: string, currentModel: string): string {
  const trimmed = (currentModel || '').trim();
  const models = getProviderModels(bucket, provider);
  if (trimmed && (models.length === 0 || models.includes(trimmed))) {
    return trimmed;
  }
  return models[0] ?? getProviderDefault(bucket, provider).model;
}

...
const AGENT_VISIBLE_FILE_ORDER = [
  'AGENT.md',
  'HEARTBEAT.md',
  'IDENTITY.md',
  'MEMORY.md',
...
const AGENT_VISIBLE_FILE_NAMES = new Set(AGENT_VISIBLE_FILE_ORDER.map((name) => name.toLowerCase()));
...
const isPreferredAgentRootFile = (file: FileInfo) => {
  const normalizedPath = normalizePath(file.path).toLowerCase();
  return normalizedPath === `agent/jiuwenclaw_workspace/${file.name.toLowerCase()}`;
};

const shouldReplaceVisibleFile = (current: FileInfo, candidate: FileInfo) => {
  const currentPreferred = isPreferredAgentRootFile(current);
  const candidatePreferred = isPreferredAgentRootFile(candidate);
  if (candidatePreferred !== currentPreferred) {
    return candidatePreferred;
  }
  const currentDepth = normalizePath(current.path).split('/').length;
  const candidateDepth = normalizePath(candidate.path).split('/').length;
  return candidateDepth < currentDepth;
};

const filterFolderData = (data: Record<string, FileInfo[]>): Record<string, FileInfo[]> => {
  const filesByName = new Map<string, FileInfo>();

  Object.entries(data).forEach(([rawFolderKey, files]) => {
    if (containsIgnoredDirectory(rawFolderKey)) return;
    files.forEach((file) => {
      if (containsIgnoredDirectory(file.path)) return;
      const key = file.name.toLowerCase();
      if (!AGENT_VISIBLE_FILE_NAMES.has(key)) return;
      const current = filesByName.get(key);
      if (!current || shouldReplaceVisibleFile(current, file)) {
        filesByName.set(key, file);
      }
    });
  });

  const rootFiles = AGENT_VISIBLE_FILE_ORDER
    .map((name) => filesByName.get(name.toLowerCase()))
    .filter((file): file is FileInfo => Boolean(file));

  return rootFiles.length > 0 ? { [ROOT_FOLDER_KEY]: rootFiles } : {};
};
    ...
    <div className="flex-1 min-h-0">
      <div className="card w-full h-full flex flex-col">
        ...
        <div className="flex-1 min-h-0 grid grid-cols-[minmax(0,3fr)_minmax(0,7fr)] gap-4">
          ...
          <div className="rounded-xl border border-border bg-card/70 backdrop-blur-sm overflow-hidden shadow-sm flex flex-col min-h-0">
            {selectedFile ? (
              <FileViewer filePath={selectedFile.path} fileName={selectedFile.name} showFilePath={false} />
            ) : (
              <>
                <div className="px-4 py-3 bg-secondary/30 border-b border-border">

...
export function FileViewer({ filePath, fileName, reloadNonce = 0, showFilePath = true }: FileViewerProps) {
          ...
          isTodoJson && jsonDerived.todoItems.length > 0 ? (
            <div className="w-full min-h-[280px] rounded-lg border border-border bg-card p-4 space-y-4">
              {(() => {
                ...
                return (
                  <>
                    {renderGroup('In progress', inProgress, 'text-accent', '▶')}
                    {renderGroup('Pending', pending, 'text-text-muted', '○')}
                    {renderGroup('Completed', completed, 'text-green-600', '✓')}
                    {renderGroup('Cancelled', cancelled, 'text-danger', '×')}
                  </>
                );
              })()}
            </div>

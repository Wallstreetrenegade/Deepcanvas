...
export function useWebSocket(options: UseWebSocketOptions): UseWebSocketReturn {
  ...
  const handleTtsPlayback = useCallback(
    (messageId: string, content: string) => {
      ...
      if (!sanitized || sanitized.startsWith('[Task interrupted]')) {
        return;
      }
    ...
    const unsubs = [
      ...
      webClient.on('chat.final', ({ payload }) => {
        ...
        if (content) {
          ...
          const isCronPlaceholderContent = /^\[cron\].*(running|executing)/i.test(content);
          ...
          if (!isCronPlaceholderContent) {
            ...
            if (!placeholderId) {
              for (let i = messages.length - 1; i >= 0; i -= 1) {
                ...
                if (/^\[cron\].*(running|executing)/i.test(msg.content)) {
  ...
  }, [apiBase, apiKey, authToken, model, projectPath, provider, setConnectionStats]);
